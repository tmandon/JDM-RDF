...
int main(int argc, char** argv)
{
    ...
    if(f)
    {
        ...
        while(line!="")
        {
            ...
            //Version jeu de test :
            if((type>=0&&type<=2&&poids>=0)||type==4)//||type>=444) //pseudorandom pour limiter le nombre de liens (&&n%10==0)
            {
        ...
        while(line!="")
        {
            ...
            if(noeuds[n1].getId()!=0&&noeuds[n2].getId()!=0)
            {
                ...
                cell = cell.substr(2,cell.length()-1);
                int type = stoi(cell);
                //Si la relation est un raffinement sémantique, alors les noeuds sont gardés pour le jeu de test.
                if(type==1)
                {
                    noeuds[n1].setPrintable('r');
                    noeuds[n2].setPrintable('a');
                }
                //poids
                getline(lineStream,cell,'|');
                cell = cell.substr(2,cell.length()-1);
    ...
    //On utilise la méthode du diko JDM, où pour un noeud, sa relation avec le poids le plus élevé sera à 1000, et les autres seront proportionelles.
    for(unsigned long int i=0;i<noeuds.size();i++)
    {
        noeud * noeud_act = &noeuds[i];
        if(noeud_act->getId()!=0)
        {
            //On cherche d'abord les poids max positif et min negatif.
            long int max_poids_p=-1, min_poids_n = 1;
            for(unsigned long int j=0;j<noeud_act->getSizeRelations();j++)
            {
                relation* rel_act = noeud_act->getRelations(j);
                int poids_act = rel_act->getPoids();
                if(poids_act>=0)
                {
                    if(poids_act>max_poids_p)
                    {
                        //Il y a dans JDM des relations avec des "faux" poids, on en tient pas compte.
                        if(poids_act<5000)
                        {
                            max_poids_p = poids_act;
                        }
                    }
                } else {
                    if(poids_act<min_poids_n)
                    {
                        //Il y a dans JDM des relations avec des "faux" poids, on en tient pas compte.
                        if(poids_act>-1000)
                        {
                            min_poids_n = poids_act;
                        }
                    }
                }
            }
            //Ensuite on dit que les poids max(min) sont égaux à (-)1000, et on mets les autres à cette proportion.
            for(unsigned long int j=0;j<noeud_act->getSizeRelations();j++)
            {
                relation* rel_act = noeud_act->getRelations(j);
                int poids_act = rel_act->getPoids();
                if(poids_act>=0)
                {
                    if(poids_act>5000)
                    {
                        rel_act->setPoids(1000);
                    } else {
                        float val = float(poids_act)/float(max_poids_p);
                        rel_act->setPoids(val*1000);
                    }
                } else {
                    if(poids_act<-1000)
                    {
                        ...
                        float val = float(poids_act)/float(min_poids_n);
                        rel_act->setPoids(val*1000);
                    }
                }
            }
    ...
    if(fichier_turtle)
    {
        ...
        for(unsigned long int i=0;i<noeuds.size();i++)
        {
            ...
            if(noeud_act->getId()!=0)
            {
                ...
                {
                    relation* rel_act = noeud_act->getRelations(j);
                    if(noeud_act->getPrintable()=='a'||rel_act->getType()==1||rel_act->getType()==999)
                    {
